...
const VoiceActivityIndicator = ({ testState, isStreaming }) => {
  ...
  // Determine colors based on who is speaking
  const primaryColor = isUserSpeaking ? 'green' : 'blue';
  const icon = isUserSpeaking ? <Mic className="w-10 h-10" /> : <Bot className="w-10 h-10" />;

  // Calculate bar heights - Enhanced for user, normal for AI
  const getBarHeight = (index, level) => {
    const baseHeight = 10;
    const maxHeight = isUserSpeaking ? 50 + (index * 15) : 35 + (index * 12); // Taller bars for user
    ...
    const multiplier = isUserSpeaking ? 1.5 : 1; // More dramatic for user
